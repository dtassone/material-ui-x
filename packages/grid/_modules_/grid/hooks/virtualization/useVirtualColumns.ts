...
export const useVirtualColumns = (
  ...
  const getColumnIdxFromScroll = React.useCallback(
    (left: number) => {
      const positions = apiRef.current.instance.getColumnsMeta().positions;
      const hasColumns = apiRef.current.instance.getVisibleColumns().length;
  ...
  const getColumnFromScroll = React.useCallback(
    (left: number) => {
      const visibleColumns = apiRef.current.instance.getVisibleColumns();
  ...
  const updateRenderedCols: UpdateRenderedColsFnType = React.useCallback(
    (containerProps: ContainerProps | null, scrollLeft: number) => {
      ...
      const visibleColumns = apiRef.current.instance.getVisibleColumns();
      const columnsMeta = apiRef.current.instance.getColumnsMeta();
